...
const transport=createServer(async(req,res)=>{
 try{
  ...
  const r=await fetch('http://localhost:38092'+req.url,{method:req.method,headers:{'x-api-key':req.headers['x-api-key']??'','content-type':'application/json'},...(body.length?{body}:{}),signal:AbortSignal.timeout(60000)});
...
try{
 ...
 const active={id:'tali-acceptance-20260908'};
 ...
 for(const mode of ['window_buffered']){
  ...
  await until(async()=>{const r=await fetch(`http://localhost:38092/runtime/v1/integrations/${resource.integrationId}/verify`,{method:'POST',headers:{'x-api-key':resource.credential,'content-type':'application/json'},body:'{}'});return r.ok?await r.json():{};},v=>v.ready);
 ...
 delete report.failure;report.passed=report.cases.length===3&&report.cases.every(c=>c.passed)&&report.quality.every(c=>c.passed);save();assert(report.passed,'Some SSE cases failed; preserve evidence and budget.');
