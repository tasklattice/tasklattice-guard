...
try{
 ...
 for(const mode of ['window_buffered']){
  ...
  await until(async()=>{const r=await fetch(`http://localhost:38092/runtime/v1/endpoints/${resource.endpointId}/verify`,{method:'POST',headers:{'x-api-key':resource.credential,'content-type':'application/json'},body:'{}'});return r.ok?await r.json():{};},v=>v.ready);
