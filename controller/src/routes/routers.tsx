...
export function RoutersPage() {
  ...
  return <section className="py-6 sm:py-8"><PageHeader title={t('流量 Routers', 'Traffic Routers')} description={t('按顺序选择流量，在每条 Route 内按比例分配给固定版本的 Guardrail。', 'Select traffic in Route order, then distribute each Route across pinned Guardrail versions.')} action={auth.user?.role === 'admin' ? <Button className="min-h-11" onClick={() => setOpen(true)}><Plus />{t('创建 Router', 'Create Router')}</Button> : undefined} />
...
function RouterRow({ router, endpointName }: { router: TrafficRouter; endpointName: (id: string) => string }) {
  ...
  return <tr className="border-b last:border-0"><td className="p-4"><Link className="inline-flex min-h-11 items-center font-medium text-primary" to="/integration/routers/$routerId" params={{ routerId: router.id }}>{router.name}</Link><p className="max-w-xs text-xs text-muted-foreground">{router.description}</p></td><td className="p-4">{router.endpointIds.length ? router.endpointIds.map(endpointName).join(', ') : t('未接入', 'Unbound')}</td><td className="p-4 tabular-nums">{router.draft.routes.filter(r => r.kind === 'normal' && r.enabled).length} + 1 Fallback</td><td className="p-4 tabular-nums">{metrics.error ? t('数据暂不可用', 'Data unavailable') : metrics.data ? metrics.data.total.toLocaleString() : '—'}</td><td className="p-4">{metrics.data ? share(fallback, metrics.data.total) : '—'}</td><td className="p-4"><RouterStatus router={router} /></td></tr>;
}
export function RouterStatus({ router }: { router: TrafficRouter }) {
  const t = useRoutingText();
  return <span className="text-sm">{router.rolloutStatus === 'failed' ? t('分发失败', 'Rollout failed') : router.rolloutStatus === 'active' ? t('已生效', 'Active') : router.rolloutStatus === 'distributing' ? t('分发中', 'Distributing') : t('未发布', 'Unpublished')}{router.activeRevision ? ` · r${router.activeRevision}` : ''}{router.draftRevision !== router.activeDraftRevision && ` · ${t('有草稿', 'Draft changes')}`}</span>;
}
