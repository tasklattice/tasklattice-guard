...
const ZH_CONTENT: HelpContent = {
  ...
  searchLabel: "搜索帮助文档",
  searchPlaceholder: "搜索 Policy、Rail、Flow、Validation、Router…",
  searchHint: "可搜索产品术语、操作步骤、运行时行为和故障处理。",
  clearSearch: "清除搜索",
  noResultsTitle: "没有匹配的帮助内容",
  noResultsDescription: "尝试更短的概念名称，例如 Rail、Action、版本或路由。",
  contents: "文档目录",
  overviewLabel: "系统背景",
  ...
  architecture: [
    { name: "Endpoint", description: "识别并认证调用 TaskLattice 的应用、Agent 或网关。" },
    { name: "Router", description: "按 Traffic Scope 为该 Endpoint 选择一个已发布的 Guardrail Version。" },
  ...
  guides: [
      ...
      articles: [
        {
          ...
          steps: [
            ...
            { title: "5. 创建 Router", description: "选择 Endpoint 和 Traffic Scope，把匹配流量绑定到确切版本。" },
      ...
      articles: [
        {
          ...
          terms: [
            ...
            { name: "创建 Router", description: "当 Guardrail 已发布，需要把某个 Endpoint 的一部分或全部流量路由到该版本时。" },
          ],
        ...
        },
        {
          ...
          steps: [
            ...
            { title: "关联调用", description: "使用稳定 call ID 关联输入与输出，保证两阶段固定到同一个版本；Endpoint 身份隔离不同网关。" },
          ],
    ...
    },
    {
      ...
      articles: [
        ...
        {
          id: "operator-routing",
          title: "Endpoint、Router 与 Traffic Scope",
          summary: "从 Endpoint 身份到 Traffic Router 路由：先确认流量身份，再按顺序匹配；不能用可伪造字段代替认证边界。",
          steps: [
            { title: "注册 Endpoint", description: "为每个应用、Agent、网关或信任边界创建独立身份和凭据。" },
            { title: "创建 Router", description: "选择已发布 Guardrail Version 和 Traffic Scope。一个 Endpoint 内按顺序 first-match-wins。" },
            { title: "保留 fallback", description: "All traffic 路由保持在最后；未匹配任何 Endpoint 路由的流量使用系统管理 baseline。" },
            { title: "验证可信上下文", description: "优先使用 Endpoint 身份、已验证 JWT claims 或由可信代理重写的 headers。" },
          ],
        ...
        },
        {
          ...
          bullets: [
            ...
            "观察 Runtime health 中活动 Router、Endpoint 和可选能力状态。",
            "回滚会验证并预热历史不可变快照，再在数据库事务中切换 Guardrail 与绑定的 Routers。",
        ...
        },
        {
          ...
          steps: [
            { title: "1. 身份与路由", description: "确认 Endpoint 已启用、凭据匹配、Router 顺序正确，Traffic Scope 使用可信字段。" },
            { title: "2. 版本状态", description: "确认 Router 指向预期 Guardrail Version，checksum 与发布记录一致。" },
...
ZH_CONTENT.glossary = [
  ...
  ["guardrail-version", "Guardrail Version", ["护栏版本"], "不可变的发布与回滚单元，包含编译 artifact、依赖和 checksum。", "Router 总是引用确切版本；回滚切换版本指针而不是重写历史。", ["user", "operator"]],
  ...
  ["router", "Traffic Router", ["路由", "Router", "route"], "把一个 Endpoint 的匹配 Traffic Scope 路由到已发布 Guardrail Version。", "同一 Endpoint 内按顺序 first-match-wins；All traffic 保持最后。", ["user", "operator"]],
  ["endpoint", "Endpoint", ["端点", "adapter"], "代表一个经过认证的应用、Agent 或网关实例。", "不同信任边界使用独立 Endpoint，避免身份、凭据和 call context 混用。", ["developer", "operator"]],
  ["traffic-scope", "Traffic Scope", ["流量范围", "路由条件"], "基于可信请求事实构造的 AND/OR 路由表达式。", "空表达式表示该 Endpoint 的全部流量；外部 headers 若未被可信代理重写可能被伪造。", ["developer", "operator"]],
...
const EN_CONTENT: HelpContent = {
  ...
  searchPlaceholder: "Search Policy, Rail, Flow, Validation, Router…",
  ...
  noResultsDescription: "Try a shorter concept such as Rail, Action, version, or router.",
  contents: "Contents",
  ...
  architecture: [
    { name: "Endpoint", description: "Identifies and authenticates the application, agent, or gateway calling TaskLattice." },
    { name: "Router", description: "Uses Traffic Scope to select a published Guardrail Version for that Endpoint." },
  ...
  guides: [
      ...
      articles: [
        {
          ...
          steps: [
            ...
            { title: "5. Deploy", description: "Bind an Endpoint and Traffic Scope to the exact version." },
    ...
    },
    {
      ...
      articles: [
        {
          id: "developer-policy-or-guardrail",
          title: "Policy vs Guardrail vs Router",
          summary: "Policies provide reusable behavior, Guardrails compose a scenario, and Routers route traffic.",
          terms: [
            ...
            { name: "Create a Router", description: "When a published Guardrail must protect an Endpoint's traffic." },
          ],
    ...
    },
    {
      ...
      articles: [
        ...
        {
          id: "operator-routing",
          title: "Endpoint, Router, and Traffic Scope",
          summary: "Authenticate traffic first, then match ordered routes. Do not use spoofable fields as a trust boundary.",
          steps: [
            { title: "Register Endpoint", description: "Create separate identity and credentials for each app, agent, gateway, or trust boundary." },
            { title: "Create Router", description: "Select a published version and Traffic Scope. Routes are first-match-wins." },
            { title: "Keep fallback", description: "All traffic stays last; unmatched traffic uses the system baseline." },
            { title: "Verify context", description: "Prefer Endpoint identity, verified JWT claims, or trusted-proxy rewritten headers." },
          ],
        ...
        },
        {
          ...
          bullets: [
            ...
            "Watch Runtime health for active Routers, Endpoints, and optional capabilities.",
            "Rollback prewarms a historical immutable snapshot, then switches Guardrail and Router pointers transactionally.",
        ...
        },
        {
          ...
          steps: [
            { title: "1. Identity and route", description: "Confirm Endpoint state, credentials, Router order, and trusted Traffic Scope fields." },
...
EN_CONTENT.glossary = ZH_CONTENT.glossary.map((entry) => {
  const translations: Record<string, Pick<GlossaryEntry, "definition" | "background">> = {
    ...
    "guardrail-version": { definition: "The immutable release and rollback unit containing compiled artifacts, dependencies, and checksum.", background: "Routers reference exact versions; rollback switches pointers instead of rewriting history." },
    ...
    router: { definition: "A route from an Endpoint's matching Traffic Scope to a published Guardrail Version.", background: "Routes are first-match-wins; All traffic stays last." },
    endpoint: { definition: "An authenticated application, agent, or gateway instance.", background: "Separate trust boundaries use separate Endpoints to isolate identity, credentials, and call context." },
    "traffic-scope": { definition: "An AND/OR route expression over trusted request facts.", background: "An empty scope means all Endpoint traffic; untrusted headers may be spoofed." },
    evidence: { definition: "Immutable records of versions, validation, router, and privacy-conscious runtime decisions.", background: "Evidence omits protected bodies while retaining Policy, Rule, version, Decision, and latency context." },
